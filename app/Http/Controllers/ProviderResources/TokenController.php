...
class TokenController extends Controller
{
    ...
    public function send_mobile(Request $request)
    {
        ...
        try {
            ...
            if ($token_user != null) {
                ...
                $message = "<#> CABI: Your verification code is " . $otp . " PUJWhJxn7T+";
                //$message = "DO NOT SHARE:".$otp." is the OTP for your account. Keep this OTP to yourself for account safety.";
                // (new SendPushNotification)->sendSMSUser($number,$message);
            }
    ...
    public function forgot_password(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json(['success' => 0, 'message' => 'This mobile number is not registered in CABI.'], 200);
        }
        try {
            $provider = Provider::where('mobile', $request->mobile)->first();
            $otp = 123456;
    ...
    public function send_otp(Request $request)
    {
        ...
        if ($new_user != null) {
            ...
            //     $token_user->save();
            // }else {
            //     return response()->json(['success' => "0", "message" => "Por favor regístrese en CABI"], 200);
            ...
            return response()->json(['success' => "0", "message" => "Please register with CABI"], 200);
        }
    }
    ...
    public function plans()
    {
        try {
            ...
            $plan = Plan::select('id as plan_id','name','description','price','currency')->where('is_active',1)->get();

            return response()->json(['plans' => $plan, 'success' => 1], 200);
    ...
    public function subscribeToPlan(Request $request)
    {
        ...
        $user = Provider::find(Auth::guard('providerapi')->user()->id);
        ...
        // Set the subscription start date (now)
        $subscriptionStart = Carbon::now();

        // Calculate subscription expiry date based on plan interval
        switch ($plan->interval) {
            case 'day':
                $subscriptionExpires = $subscriptionStart->copy()->addDay();
                break;
            case 'week':
                $subscriptionExpires = $subscriptionStart->copy()->addDays(7);
                break;
            case 'month':
                $subscriptionExpires = $subscriptionStart->copy()->addMonth();
                break;
            case 'year':
                $subscriptionExpires = $subscriptionStart->copy()->addYear(); 
                break;
            case 'minute':
                $subscriptionExpires = $subscriptionStart->copy()->addHour();
                break;
            default:
                return response()->json(['error' => 'Invalid plan interval.'], 400);
        }
