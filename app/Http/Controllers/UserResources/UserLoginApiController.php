...
class UserLoginApiController extends Controller
{
    ...
    public function signup(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['success' => "1", "message" => 'You have successfully registered on CABI'], 200);
        } catch (Exception $e) {
            return $e;
            return response()->json(['error' => trans('api.something_went_wrong')], 500);
        }
    ...
    public function login(Request $request)
    {
        ...
        try {
            ...
            if ($User == null) {
                return response()->json(['success' => 0, "message" => "Account details not found. Please contact CABI "], 200);
            } else {
                $password = $request->password;
            }

            if ($User->account_status != 'approved') {
                return response()->json(['error' => 'Your account has been banned Please contact CABI.', 'success' => 0]);
            }
    ...
    public function send_mobile(Request $request)
    {
        ...
        //     $User = User::where('mobile','=',$request->mobile)->first();
        //     if($User == null){
        //         return response()->json(['success' => 0, "message"=> "Account details not found. Please register with CABI "], 200);
        //     }
    ...
    public function otp_verify(Request $request)
    {
        ...
        $User = User::where('mobile', '=', $request->mobile)->first();
        if ($User == null) {
            return response()->json(['success' => 0, "message" => "Account details not found. Please register for CABI "], 200);
        }
        if ($request->device_token == 'COULD NOT GET FCM TOKEN') {
            return response()->json(['success' => "0", "message" => 'COULD NOT GET FCM TOKEN'], 200);
        }
